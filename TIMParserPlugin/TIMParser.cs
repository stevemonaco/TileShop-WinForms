...
namespace TIMParserPlugin
{
    ...
    public class TIMParser : IFileParserContract
    {
        ...
        private int SearchTIMFile(string FileName)
        {
            ...
            try
            {
                FileStream fs = new FileStream(FileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite, 65536, FileOptions.SequentialScan);
                ...
                for(int i = 0; i < fs.Length / 4; i++)
                {
                    ...
                    if (!FileAdded) // TODO: Modify when FileManager is changed to not have files always opened
                    {
                        FileManager.Instance.AddFileStream(FileName, fs);
                        FileAdded = true;
                    }
